...
void Population :: PrintL(string filename){
  ofstream Output;
  Output.open(filename, ios::app);
  Output << m_Matr[0].GetL() << "\t" << m_Matr[1].GetL() << endl;
...
void Population :: PrintBestPath(string filename){
  ...
  for(int i=0; i<m_dim; i++){
    Output << m_Matr[0].GetVector()[i] << "\t" << m_pos[0][m_Matr[0].GetVector()[i]] << "\t" << m_pos[1][m_Matr[0].GetVector()[i]] << endl;
  }
  Output << m_Matr[0].GetVector().size() << "\t" << m_pos[0][0] << "\t" << m_pos[1][0] << endl;
}
...
void Population::Accept(){
  double alpha=fmin(1., exp(-1./m_temp*(m_Matr[1].GetL()-m_Matr[0].GetL())));
  double r=m_rand->Rannyu();
  if(r<alpha){
    m_Matr[0]=m_Matr[1];
    m_accepted = m_accepted +1;
  }else{
    m_Matr[1]=m_Matr[0];
  }
  m_gen=m_gen+1;
...
void Population::SetTemp(double ti, double tf){
  
 // double T=0.5*exp(double(i)/-50000.);
  
  double m=0.99999;
  m_temp*=m;
  ofstream Output;
  Output.open("Temp.out", ios::app);
  Output << m_temp << endl;
}
