...
void MeasureR(int i){ //Properties measurement
  int bin;
  double v, t, vij;
  double dx, dy, dz, dr;
  ofstream Epot, Ekin, Etot, Temp;
  
  Epot.open("output_epot" + to_string(i) + ".dat",ios::app);
  Ekin.open("output_ekin" + to_string(i) + ".dat",ios::app);
  Temp.open("output_temp" + to_string(i) + ".dat",ios::app);
  Etot.open("output_etot" + to_string(i) + ".dat",ios::app);
...
void MeasureRB(int i, int N, std::vector<std::vector<double> > &M)
...
void Restart(){
  ...
  if (restart==false) ConfFinal();
      ...
      //Scrivo i risultati della simulazione precedente
        ...
        ofstream WriteConf;
        WriteConf.open("old" + to_string(count) +".final");
        for (int i=0; i<npart; ++i){
          WriteConf << xold[i]/box << "   " <<  yold[i]/box << "   " << zold[i]/box << endl;
        }
        WriteConf.close();
     
        WriteConf.open("old" + to_string(count) +".0");
        for (int i=0; i<npart; ++i){
          WriteConf << x[i]/box << "   " <<  y[i]/box << "   " << z[i]/box << endl;
        }
        WriteConf.close();
        ...
        for(int istep=1; istep <= nstep; ++istep){
          ...
          if(istep%iprint == 0) cout << "Number of time-steps: " << istep << endl;
          if(istep%10 == 0){
            MeasureR(count);     //Properties measurement
            //        ConfXYZ(nconf);//Write actual configuration in XYZ format //Commented to avoid "filesystem full"!
            nconf += 1;
          }
        }
...
void FinalRestart(){
  ...
  if (restart==false) ConfFinal();
    ...
    ofstream WriteConf;
    WriteConf.open("old" + to_string(nSimulations) +".final");
    for (int i=0; i<npart; ++i){
      WriteConf << xold[i]/box << "   " <<  yold[i]/box << "   " << zold[i]/box << endl;
    }
    WriteConf.close();
    
    WriteConf.open("old" + to_string(nSimulations) +".0");
    for (int i=0; i<npart; ++i){
      WriteConf << x[i]/box << "   " <<  y[i]/box << "   " << z[i]/box << endl;
    }
    WriteConf.close();
    ...
    for(int i=0; i<N ; i++){
      ...
      for(int j=0; j<L; j++){
        ...
        if(j%10 == 0){ //misura i valori ogni 10 passi
          MeasureRB(nSimulations, i, M); //Properties measurement
    ...
    Eval_Print("ave_epot.out", M[0], M[1], N);
    Eval_Print("ave_ekin.out", M[2], M[3], N);
    Eval_Print("ave_temp.out", M[4], M[5], N);
    Eval_Print("ave_etot.out", M[6], M[7], N);
